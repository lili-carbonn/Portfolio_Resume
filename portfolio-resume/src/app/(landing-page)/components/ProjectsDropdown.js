  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="text-foreground relative px-2 py-1 overflow-hidden group flex items-center"
      >
        <span className="relative z-10 transition-all duration-300 group-hover:text-primary-600">
          Projects
        </span>
        <ChevronDownIcon 
          className={`ml-1 h-4 w-4 transition-all duration-300 group-hover:text-primary-600 ${isOpen ? 'rotate-180' : ''}`} 
        />
      ...
      {isOpen && (
        <div 
          className="absolute mt-2 w-64  rounded-lg shadow-xl backdrop-blur-md border bg-amber-50 bg-opacity-70 border-primary-100 z-20 overflow-hidden animate-fadeIn origin-top"
        >
          <div className="py-1">
            <Link
              href="/#projects"
